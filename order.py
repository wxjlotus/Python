...
for file in filenames:
    ...
    dftemp=dftemp[['商品名称(name)','订单总额(final_amount)','订单实付金额(payed)','订单折扣优惠(discount)','订单现金券减免(cpns_money)','订单油箱抵用金额(goil_money)']]
    df=pd.concat([dftemp,df])
print ("df.shape after concat: ",df.shape)


print (df[['订单总额(final_amount)', '订单实付金额(payed)', '订单折扣优惠(discount)','订单现金券减免(cpns_money)','订单油箱抵用金额(goil_money)']].sum()/10000)
