...
# -*- coding:utf-8 -*-
#win7中文版64位运行通过；
#日期罗列方式计算总盈亏，数据来源是储油列表、交易列表、当前价格列表,无法解决逐笔对冲问题
#计算程序运行时间
import time
import pandas as pd
import os
...
df1=dfall[dfall['卡类型']!='大众储油卡(新)']

df2=dfall[dfall['卡类型']=='大众储油卡(新)']


ratio=pd.DataFrame([0.2,0.25,0.3,0.35],index=[100,200,400,800],columns=['Ratio'])
df3=pd.merge(df2,ratio,how='left', on=None,left_on=['销售升数'],right_index=True)
df3['total PL']=(-df3['销售升数']*(df3['当前单价']-df3['销售单价'])+df3['兑付升数']*(df3['当前单价']-df3['兑付单价']))*df3['Ratio']
df3.drop('Ratio',axis=1,inplace=True)

dfall=pd.concat([df1,df3])
...
df4=dfall.groupby(['日期','卡类型', '地区', '油品','交易方向'])['销售金额', '销售升数','赠送升数','兑付金额','兑付升数','Total Value', 'position', 'total PL'].sum().reset_index()
print ("销售数量吨： ",df4['销售升数'].sum()/1270)
print ("兑付数量吨： ",df4['兑付升数'].sum()/1270)
