...
#筛选兑付
#dfcash1=dfcash.iloc[:,[0,3,4,5,10]]
#dfcash=dfcash[dfcash["交易类型(trade_type)"]=='兑付']
#dfcash=dfcash[['储油通卡号(card_bn)','产品名称(product_name)','交易类型(trade_type)','数量(trade_nums)','金额(income)','交易成功时间(trade_end_time)']]
print ("dfcash.shape after filter: ",dfcash.shape)

print (dfcash['金额(income)'].sum()/10000)

os.chdir(r"D:\workspace") 
dfcash.to_csv('cash2016-20170601.csv',index=False)
